function reviewWeekBotPost() {
  ...
  function postToSlack_() {
    ...
    let last_shift_review = sheet.getSheetValues(3, 3, 1, 1);
    let last_shift_gai = sheet.getSheetValues(3, 4, 1, 1);
    ...
    let last_time_review = sheet.getSheetValues(7, 5, 1, 1);
    ...
    if (last_shift_total > 0) {
      sendHttpPost_('<@slackのユーザID>さん、こんにちは！週刊通知botです！\
                  ...
                  \n実働時間（レビ）： 残り *' + last_time_review + '* 時間\
                  ...
                  \nレビ１時間あたり： *' + every_hour_review + '* 件 ' + lank_review_alert + ' \
