function kisoWeekBotPost() {
  ...
  // 使用するシートを取得
  let sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('基礎→応用（雛形）'); 
  ...
  function postToSlack_() {
    ...
    let ouyo_goal = sheet.getSheetValues(7, 4, 1, 1); 
    let ouyo_now = sheet.getSheetValues(7, 3, 1, 1);
    var last_day_ave = sheet.getSheetValues(10, 2, 1, 1); 
    var last_ouyo_ave = sheet.getSheetValues(10, 3, 1, 1); 
    ...
    if (Math.sign(last_ouyo_ave) == -1) {
      var last_ouyo_ave = "clear!";
    } else {
      true
    };
    // シフト残数あり
